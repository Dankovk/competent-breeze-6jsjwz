...
import { type BabylonPlugin, type BabylonUniRenderer } from "@geenee/bodyrenderers-babylon";
...
import { type SKUKeys, skuTypeMap, SwiperOptionsConfig, BodyExperienceTypeNames } from "@/utils/skuHelper";
...
const isIOS = isIos();
const isSafari = /^((?!chrome|android).)*(safari|sheinapp)/i.test(navigator.userAgent);

let engineInitParams: Record<string, any> = {};
let engine = null;
let Renderer = null;

const sdkToken = (isAds && process.env.SDK_TOKEN_ADS) ? process.env.SDK_TOKEN_ADS : process.env.SDK_TOKEN;
...
let notifyPlugin: FaceStateNotifier | PoseStateNotifier = new FaceStateNotifier(notifyHandler);
...
if (experienceType in BodyExperienceTypeNames) {
    ...
    countDownMessage = "Step back so that your entire body is fully within the frame.";
} else {
...
export interface StoreType {
    ...
    renderer: AvatarPoseRenderer | BagRenderer | FaceRenderer | HatRenderer | HeadRenderer | EarringsRenderer | null
    ...
    setupEngineParams: () => { size?: { width: number, height: number }, rear: boolean } | MediaStream
...
export const store: StoreType = new Proxy<StoreType>({
    ...
    listeners,
    setupEngineParams () {
        return Reflect.get(store, "isMobile") ? { rear: Reflect.get(store, "isRear") } : { size: { width: 1920, height: 1080 }, rear: Reflect.get(store, "isRear") };
    },
